...
void runGame()
{
	...
	if (winner == "C")
	{
		cout << "It was Draw game! NO WINNER!" << endl;
	}
	else
	{
...
}// end of initializing game board
...
void printCurrentBoard(string gameBoard[ROWS][COLS])
{
	...
	cout << endl; //extra spacing
}// end of, print the current board
...
string getWinner(string gameBoard[ROWS][COLS])
{
	...
	{
		if (gameBoard[0][i] != " " && gameBoard[i][0] == gameBoard[i][1] && gameBoard[i][1] == gameBoard[i][2])
		{
			return gameBoard[0][i]; //we have a match (vertically)!
		}
	}//end for
